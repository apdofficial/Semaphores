...
void *tSingleQueue(void *ptr) {
    ...
    while (true) {
        ...
        vector_push_back(queue.singleQueue, 1);
...
void *tnormalQueue(void *ptr) {
    ...
    while (true) {
        ...
        vector_push_back(queue.normalQueue, groups);
        sem_post(&MutexRollerCoaster);
        queue.normalTotal += groups;
 ...
 * Function: pop_Queue
 * ----------------------------
 *   Pops last person from the Queue type based on the type if 1 normalQueue if 0 singleQueue
 *   returns: true
 */
bool pop_Queue(const int *item, const int type) {
    ...
    printf("  |%d|", *item);
    if (type == 1) {
        vector_pop_back(queue.normalQueue);
        queue.normalTotal -= *item;
    } else if (type == 2) {
        vector_pop_back(queue.singleQueue);
        queue.singleTotal -= *item;
...
bool fillRollerCoaster(void) {
    if (vector_empty(queue.singleQueue) == 0 || vector_empty(queue.normalQueue) == 0) {
        if(vector_empty(queue.normalQueue) == 0) {
            if (fireSingleRiderMode) {
                if ((freeSeats - *vector_end(queue.normalQueue, 1)) >= 0)
                    pop_Queue(vector_end(queue.normalQueue, 1),1);
                while (freeSeats - *vector_end(queue.singleQueue, 1) >= 0 && *vector_end(queue.normalQueue, 1) > 0 ) {
                    pop_Queue(vector_end(queue.singleQueue, 1),2);
                }
            ...
            if ((freeSeats - *vector_end(queue.normalQueue, 1)) >= 0) {
                return pop_Queue(vector_end(queue.normalQueue, 1), 1);
            } else if (((*vector_end(queue.normalQueue, 1) == 3) && (freeSeats - 2 == 0)) ||
                       (*vector_end(queue.normalQueue, 1) == 2 && *vector_end(queue.normalQueue, 2) == 2)) {
                int i = 0;
                for (int *it = vector_begin(queue.normalQueue); it != vector_end(queue.normalQueue, 0); ++it) {
                    if (*it == 2) {
                        freeSeats -= 2;
                        printf("  |%d|", *vector_end(queue.normalQueue, 1));
                        vector_erase(queue.normalQueue, i);
                        return true;
                    }
                    ++i;
                }
        ...
        if(vector_empty(queue.singleQueue) == 0) {
            if ((vector_size(queue.normalQueue) == 0 && (freeSeats - *vector_end(queue.singleQueue, 1)) >= 0) ||
                (freeSeats - *vector_end(queue.singleQueue, 1)) >= 0) {
                return pop_Queue(vector_end(queue.singleQueue, 1), 2);
            }
        }
    }
    return true;
    ...
    while (true) {
        ...
        while (notFull) {
            ...
            if (freeSeats == 0){
                ...
                if (vector_empty(queue.singleQueue) != 0 && vector_empty(queue.normalQueue) != 0 && freeSeats < 6) {
                    notFull = false;
                    needPeople = true;
                    sem_post(&MutexPeople);
                    usleep(50 * 1000);
                }else if (vector_empty(queue.singleQueue) != 0 && vector_empty(queue.normalQueue) != 0 && freeSeats == MAX_PEOPLE_LIMIT) {
                    needPeople = true;
                    sem_post(&MutexPeople);
                    usleep(50 * 1000);
